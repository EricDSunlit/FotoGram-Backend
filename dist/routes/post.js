...
//Obtener los pots paginados
postRoutes.get('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    let pagina = Number(req.query.pagina) || 1;
    let skip = pagina - 1;
    skip = skip * 10;
    const posts = yield post_model_1.Post.find().sort({ _id: -1 }).limit(10).populate('usuario', '-password').skip(skip).exec();
    res.json({
        ok: true,
        pagina,
        posts
...
postRoutes.post('/', [autenticacion_1.verificaToken], (req, res) => {
    ...
    post_model_1.Post.create(body).then((postDB) => __awaiter(void 0, void 0, void 0, function* () {
        yield postDB.populate('usuario', '-password'); //Tomar la relacion establecida en el modelo y llenar usuario con dicha info
        res.json({
            ok: true,
            post: postDB
        });
...
postRoutes.post('/upload', [autenticacion_1.verificaToken], (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!file) {
        return res.status(400).json({
            ok: false,
            mensaje: 'No se ha subido ningun archivo - image'
        });
    }
